...
def define_dense_model_single_layer(input_length, activation_f='sigmoid', output_length=1):
    ...
    model = keras.Sequential([
        layers.Dense(units=output_length, input_dim=input_length, activation=activation_f)
    ])
    return model
...
def define_dense_model_with_hidden_layer(input_length, 
    ...
    model = keras.Sequential([
        layers.Dense(units=hidden_layer_size, input_dim=input_length, activation=activation_func_array[0]),
...
def evaluate_mnist_model(x_test, y_test, model):
    ...
    """
    loss, accuracy = model.evaluate(x_test, y_test) # model evaluate
